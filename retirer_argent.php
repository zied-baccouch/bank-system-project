...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['num_cin']) && isset($_POST['montant'])) {
    ...
    if ($userResult->num_rows > 0) {
        // Le numéro CIN existe, vérifier le solde avant de retirer
        $getUserSoldeSql = "SELECT solde FROM users WHERE num_cin = '$numCIN'";
        $soldeResult = $conn->query($getUserSoldeSql);
        
        if ($soldeResult->num_rows > 0) {
            $row = $soldeResult->fetch_assoc();
            $solde = $row['solde'];

            if ($montantRetirer <= $solde) {
                // Le montant à retirer est inférieur ou égal au solde, effectuer le retrait
                $sql = "INSERT INTO retirer (num_cin, montant, date) VALUES ('$numCIN', '$montantRetirer', '$date')";
                $conn->query($sql);

                // Mettre à jour le solde dans la table 'users'
                $updateSoldeSql = "UPDATE users SET solde = solde - '$montantRetirer' WHERE num_cin = '$numCIN'";
                $conn->query($updateSoldeSql);

                $successMessage = "Retrait effectué avec succès! Solde mis à jour.";
            } else {
                $errorMessage = "Erreur: Le montant à retirer dépasse le solde disponible.";
            }
        } else {
            $errorMessage = "Erreur: Impossible de récupérer le solde de l'utilisateur.";
        }
    } else {
...
<div class="container">
    ...
    <form action="" method="post">
        ...
        <button type="submit" class="btn btn-primary">Retirer Montant</button>
    </form>
